...
║    T     — show performance table   A — run all algorithms                    ║
║                                                                               ║
...
class SortingTimer:
    ...
    def measure(self, algorithm_name, sort_function, shelf):
        ...
        Args:
            ...
            sort_function (callable): Функция сортировки
            shelf (Shelf): Полка с блоками для сортировки
    ...
    def get_average(self, algorithm_name):
        """Возвращает среднее время выполнения алгоритма"""
        if algorithm_name in self.history and self.history[algorithm_name]:
            return sum(self.history[algorithm_name]) / len(self.history[algorithm_name])
        return 0.0
    
    def get_best(self, algorithm_name):
        """Возвращает лучшее время выполнения алгоритма"""
        if algorithm_name in self.history and self.history[algorithm_name]:
            return min(self.history[algorithm_name])
        return 0.0
    
    def get_worst(self, algorithm_name):
        """Возвращает худшее время выполнения алгоритма"""
        if algorithm_name in self.history and self.history[algorithm_name]:
            return max(self.history[algorithm_name])
        return 0.0
    
    def get_runs_count(self, algorithm_name):
        """Возвращает количество запусков алгоритма"""
        return len(self.history.get(algorithm_name, []))
    ...
    def generate_table(self):
        ...
        """
        if not self.history:
            return "Нет данных для отображения. Запустите алгоритмы сортировки."
        
        algorithms = list(self.history.keys())
        
        # Заголовок таблицы
        table = "╔══════════════════════╦═══════════╦═══════════╦═══════════╦═══════════╦═══════════╗\n"
        table += "║ Алгоритм             ║ Запусков  ║ Среднее   ║ Лучшее    ║ Худшее    ║ Последнее ║\n"
        table += "╠══════════════════════╬═══════════╬═══════════╬═══════════╬═══════════╬═══════════╣\n"
        
        # Данные таблицы
        for algo in sorted(algorithms):
            runs = self.get_runs_count(algo)
            avg = self.get_average(algo)
            best = self.get_best(algo)
            worst = self.get_worst(algo)
            last = self.history[algo][-1] if self.history[algo] else 0.0
            
            # Форматирование названия алгоритма
        ...
        table += "💡 Для сброса статистики перезапустите программу.\n"
    ...
    def generate_comparison_chart(self):
        ...
        # Находим максимальное среднее время для масштабирования
        max_avg = max(self.get_average(algo) for algo in self.history.keys())
        
        for algo in sorted(self.history.keys()):
            avg = self.get_average(algo)
            runs = self.get_runs_count(algo)
            
            if max_avg > 0:
                bar_length = int((avg / max_avg) * 30)
            else:
                bar_length = 0
...
class Shelf(list):
    ...
    def get_state(self):
        """Возвращает текущее состояние полки (размеры блоков)"""
        return [block.size for block in self]
    
    def restore_state(self, state):
        """Восстанавливает состояние полки из списка размеров"""
        # Очищаем полку
        self.clear()
        clear()
...
def show_performance_table():
    """Отображение таблицы с результатами измерений"""
    disable_keys()
    clear()
    
    # Показываем таблицу
    table = timer.generate_table()
    chart = timer.generate_comparison_chart()
    ...
    # Показываем на экране упрощенную версию
    goto(0, 200)
    write("📊 Таблица результатов выведена в консоль", align="center", font=("Arial", 14, "bold"))
    
    goto(0, 150)
    if timer.history:
        write("Последние результаты (сек):", align="center", font=("Arial", 12, "normal"))
        y_pos = 120
        ...
        goto(0, 100)
        write("Нет данных. Запустите алгоритмы сортировки.", align="center", font=("Arial", 12, "normal"))
    
    goto(0, -200)
    write("Нажмите любую клавишу для продолжения...", align="center", font=("Arial", 11, "italic"))
...
def run_all_algorithms():
    """Запускает все алгоритмы сортировки последовательно"""
    disable_keys()
    
    algorithms = [
        ("Bubble Sort", bubble_sort, 'b'),
        ("Insertion Sort", insertion_sort, 'i'),
        ("Selection Sort", selection_sort, 's'),
        ("Quicksort", quick_sort_wrapper, 'q'),
        ("Shell Sort", shell_sort, 'h'),
        ("Cocktail Sort", cocktail_sort, 'c'),
        ("Merge Sort", merge_sort, 'm')
    ]
    ...
    clear()
    goto(0, 200)
    write("🚀 Запуск всех алгоритмов...", align="center", font=("Arial", 14, "bold"))
    update()
    time.sleep(1)
    
    results = []
    
    for i, (name, sort_func, _) in enumerate(algorithms):
        # Восстанавливаем исходное состояние
        s.restore_state(original_state)
        
        # Показываем текущий алгоритм
        clear()
        goto(0, 200)
        write(f"⏳ {name} ({i+1}/{len(algorithms)})", align="center", font=("Arial", 14, "bold"))
        goto(0, 150)
        write("Измерение времени выполнения...", align="center", font=("Arial", 12, "normal"))
        update()
        time.sleep(0.5)
        
        # Запускаем сортировку с измерением времени
        if name == "Quicksort":
            elapsed = timer.measure(name, lambda shelf: quick_sort(shelf, 0, len(shelf) - 1), s)
        else:
        ...
        clear()
        goto(0, 200)
        write(f"✅ {name} завершён", align="center", font=("Arial", 14, "bold"))
        goto(0, 150)
        write(f"Время: {elapsed:.4f} сек", align="center", font=("Arial", 12, "normal"))
        update()
        time.sleep(1)
    ...
    clear()
    goto(0, 200)
    write("📊 Все алгоритмы выполнены!", align="center", font=("Arial", 14, "bold"))
    ...
    goto(0, -100)
    write("Подробная таблица в консоли", align="center", font=("Arial", 11, "italic"))
    goto(0, -150)
    write("Нажмите SPACE для продолжения", align="center", font=("Arial", 11, "normal"))
    
    # Выводим таблицу в консоль
    print("\n" + "=" * 80)
    print("              📊 РЕЗУЛЬТАТЫ ЗАПУСКА ВСЕХ АЛГОРИТМОВ")
    print("=" * 80)
    for name, elapsed in results:
        print(f"  {name:20} : {elapsed:.4f} сек")
    print("=" * 80)
    print(timer.generate_table())
...
def start_bubble():
    ...
    original_state = s.get_state()
    elapsed = timer.measure("Bubble Sort", bubble_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    enable_keys()
    print(f"[TIMER] Bubble Sort: {elapsed:.4f} сек")
...
def start_insertion():
    ...
    elapsed = timer.measure("Insertion Sort", insertion_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Insertion Sort: {elapsed:.4f} сек")
...
def start_selection():
    ...
    elapsed = timer.measure("Selection Sort", selection_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Selection Sort: {elapsed:.4f} сек")
...
def start_quick():
    ...
    clear()
    show_text("Quicksort")
    elapsed = timer.measure("Quicksort", quick_sort_wrapper, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Quicksort: {elapsed:.4f} сек")
...
def start_shell():
    ...
    elapsed = timer.measure("Shell Sort", shell_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Shell Sort: {elapsed:.4f} сек")
...
def start_cocktail():
    ...
    elapsed = timer.measure("Cocktail Sort", cocktail_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Cocktail Sort: {elapsed:.4f} сек")
...
def start_merge():
    ...
    elapsed = timer.measure("Merge Sort", merge_sort, s)
    clear()
    show_text(instructions1)
    show_text(instructions2, line=1)
    show_text(instructions3, line=2)
    ...
    write(f"⏱️ Время выполнения: {elapsed:.4f} сек", align="center", font=("Arial", 11, "normal"))
    enable_keys()
    print(f"[TIMER] Merge Sort: {elapsed:.4f} сек")
...
instructions2 = "press h:shell  c:cocktail  m:merge  r:randomize  a:run all"
instructions3 = "t:show table  space:quit  (orange=compare, purple=move)"


if __name__ == "__main__":
    print("""
╔═══════════════════════════════════════════════════════════════════════════════╗
║                         SORTING ANIMATION v2.0                                ║
║                                                                               ║
║  Author: Dupley Maxim Igorevich - AGLA                                        ║
║  ORCID:  https://orcid.org/0009-0007-7605-539X                                ║
║  GitHub: https://github.com/QuadDarv1ne/                                      ║
║  Date:   19.04.2026                                                           ║
║                                                                               ║
║  Performance timer enabled. Results will be displayed in console.             ║
╚═══════════════════════════════════════════════════════════════════════════════╝
    """)
    msg = main()
    mainloop()